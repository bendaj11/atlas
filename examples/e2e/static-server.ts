...
createServer(async (request, response) => {
  try {
    ...
    const fallback =
      options.spa && !extname(requestPath)
...
function parseArguments(values) {
  ...
  if (!flags.root || !flags.port)
    throw new Error(
      'Usage: static-server --root=<directory> --port=<port> [--spa]',
    );
  return {
    root: flags.root,
...
function safePath(rootDirectory, requestPath) {
  ...
  if (
    ...
    throw new Error('Path escapes the server root.');
  return candidate;
}
...
async function resolveFile(candidate, fallback) {
  try {
    ...
    if (details.isDirectory())
      return resolveFile(join(candidate, 'index.html'), fallback);
  } catch {}
  if (fallback && candidate !== fallback)
    return resolveFile(fallback, undefined);
  return undefined;
}
