...
@Component({ selector: "atlas-dashboard-angular-root", standalone: true, template: `<h1>Dashboard Angular</h1><p>Mounted at {{ context.basePath }}</p><button type="button" (click)="openToast()">Show toast</button><button type="button" (click)="openReactPopup()">Open React widget popup</button>` })
class AtlasMfRootComponent {
  readonly context = inject(ATLAS_MF_CONTEXT);
  private readonly atlas = injectAtlasSdk<HostExtensions>();
  openToast() { this.atlas.toast.open({ title: `Project ${this.atlas.hostData.projectId} is ready` }); }
