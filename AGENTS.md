...
- Create a colocated driver when the unit has boundaries to arrange or observe (mocks, storage, browser APIs, DOM, listeners registered at import). A pure function has no driver; its spec calls it directly. The spec always invokes the unit under test itself; a driver never wraps that call or stores its result or error. Keep specs declarative: mock setup, rendering, interaction, and semantic queries belong in the driver under `given`, `when`, and `get`.
...
- DOM units run under jsdom (`/** @jest-environment jsdom */`) with the real `document`; never element fakes cast to DOM types.
- A unit declares the dependencies it reads (`Pick<ClusterDependencies, ...>`); drivers pass exact shapes, never `as unknown as XDependencies`.
