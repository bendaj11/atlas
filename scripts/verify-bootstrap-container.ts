...
try {
  ...
  const user = (
    await docker(['inspect', '--format', '{{.Config.User}}', container])
  ).trim();
  if (!user)
    throw new Error('Bootstrap container must run as a non-root image user.');
  ...
  if (!deepLink.includes('atlas-host-root'))
    throw new Error('Container did not apply SPA fallback.');
