...
| `src/entry.ts` | The generated Atlas mount entry for the Angular MF. It exports the lifecycle Atlas loads through Native Federation and wires the MF to the host SDK and inner Angular routing. Edit it only when changing Atlas lifecycle wiring or the MF root router setup. |
...
| `federation.config.js` | Angular Native Federation compatibility wiring generated by Atlas. Atlas uses it to expose the MF entry and widgets. Keep product configuration in `atlas.config.ts`. |
