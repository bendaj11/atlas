...
- `src/entry.ts`: the generated Atlas mount entry for the Angular MF. It exports
  ...
  host SDK and inner Angular routing. Edit it only when changing Atlas lifecycle
  wiring or the MF root router setup.
- `src/app/app.component.ts`: the main routed Angular component. Keep this as
  the app root, and add feature screens in folders under `src/app`.
- `src/app/routes.ts`: the Angular route tree. It connects app screens such as
  `home/` and `details/`.
