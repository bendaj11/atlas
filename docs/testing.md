...
2. Builds the Angular and React example MFs through `atlas build`.
...
7. Publishes two immutable MF releases, rolls the live catalog backward and
   forward, and proves the same prebuilt host loads each selected release.

The suite verifies Angular MFs in React hosts, React MFs in Angular hosts, framework-native inner routing, cross-framework widgets, popups, opt-in loading UI, failed-remote fallback UI, CORS, and mutable versus immutable cache headers.
...
```sh
yarn playwright test -g "Angular host mounts a React MF"
