...
atlas dev orders --host=customer-host --host-url=http://localhost:4200/orders
```
...
Run the host separately with `npm run dev` from the host project first. Run
`atlas dev` from the directory that contains both generated projects, or from
your monorepo root. Then open the printed **Open host** URL from `atlas dev`. The
host discovers the override from its `atlas-override` query parameter, while
every other app continues to come from the production catalog. No Native
