...
This guide creates an Angular host named `customer-host` and an independently
deployed Angular app named `orders`. You will mount the app at `/orders`, run it
...
| `customer-host/src/bootstrap.ts` | Starts Atlas and connects Angular Router, federation, and host services |
| `customer-host/src/app/app.component.ts` | Product layout and Atlas mount anchors |
| `customer-host/federation.config.js` | Generated Native Federation compatibility wiring |
...
Keep the hidden Angular `router-outlet`; it aligns Angular Router with Atlas
route ownership. Atlas apps mount in `data-atlas-route-outlet`, not in that
hidden outlet.

In `src/bootstrap.ts`, replace generated placeholders with real auth, HTTP,
toast, modal, host data, and monitoring services. Apps consume these services
through typed Atlas SDK contracts instead of importing host source.

Detailed host examples: [Angular host guide](host-getting-started.md),
[routing](routing.md), and [SDK](sdk.md).
...
| `orders/src/entry.ts` | Atlas lifecycle entry connecting Angular, scoped routing, and host SDK |
| `orders/src/main.ts` | Native Federation dev-server initialization; it does not bootstrap the app |
| `orders/src/app/app.component.ts` | Feature root component |
| `orders/src/app/routes.ts` | App-owned inner routes |
...
Build normal Angular components and inner routes under `src/app`. Use the Atlas
SDK for host services and cross-app navigation. Use imported or relative asset
URLs, never host-root `/assets/...` paths.

Detailed app examples: [Angular app guide](app-getting-started.md),
...
4. Manifest points to immutable Angular assets.
...
Deploy the Angular browser output using your normal host pipeline. Production
server must:
...
`atlas build` runs the normal Angular production build, creates a manifest,
...
Rollback selects an older immutable build. It does not rebuild Angular, delete
the failed build, or redeploy the host.
...
Use [Angular troubleshooting](troubleshooting.md) for framework symptoms and
