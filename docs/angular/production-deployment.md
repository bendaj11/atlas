...
This guide takes an Angular Atlas app from source code to a production remote
loaded by an unchanged host.

## Host Domain

The Angular host serves `public/atlas.runtime.json` as a static file:
...
Atlas runs the Angular build, creates the app manifest, fingerprints assets,
and writes upload plans under `dist/atlas-publication`.

Do not upload Angular source files, workspace config, or local development
artifacts. Upload only the files listed by the Atlas publication plan.
