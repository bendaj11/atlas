...
Deploy generated `dist/bootstrap` with Nginx or equivalent static hosting. Routine host and app publication uses native workspace `atlas:publish` targets. Catalog activation changes selected UI without rebuilding bootstrap container. Follow [Angular production deployment](production-deployment.md).
