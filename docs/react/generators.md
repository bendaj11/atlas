...
| `src/main.tsx`         | Host team      | Vite asset-server entry. It directs developers to `atlas dev`; only Atlas bootstrap supplies required runtime configuration.           |
| `src/bootstrap.tsx`    | Atlas/platform | Federated `mount` from `defineReactHost()`; Atlas owns root, router, provider, and federation. Do not edit.                            |
| `src/host-layout.tsx`  | Host team      | Replaceable product layout with Atlas anchors.                                                                                         |
| `src/host.config.tsx`  | Host team      | Product SDK capabilities and host runtime customization.                                                                               |
...
`atlas.config.ts`. Atlas mounts `src/bootstrap.tsx` remotely and provides
SDK/runtime contexts; generated apps do not self-boot.
