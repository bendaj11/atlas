...
function stylableVitePlugin(): Plugin {
  ...
  return {
    ...
    load(id) {
      return isStylableModule(id)
        ? readFileSync(stylablePath(id), 'utf8')
        : null;
    },
    transform(source, id) {
...
function stylablePath(id: string): string {
  return Buffer.from(id.slice('\0stylable:'.length), 'base64url').toString(
    'utf8',
  );
}

function plainImporter(importer: string): string {
  return importer.startsWith('\0stylable:')
    ? stylablePath(importer)
    : importer.split('?')[0];
}
