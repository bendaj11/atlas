...
type PayloadEventKey<TEvents extends object> = Exclude<
  EventKey<TEvents>,
  PayloadlessEventKey<TEvents>
>;
type StoredEventListener<TEvents extends object> = (
...
function getOrCreateSubscribers<TEvents extends object>(
  ...
  const subscribers =
    listeners.get(type) ?? new Set<StoredEventListener<TEvents>>();
