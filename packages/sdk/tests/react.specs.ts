...
test("React generator emits React 19 Vite Native Federation projects", () => {
  ...
  assert.match(host.get("src/atlas-bootstrap.ts"), /AtlasDefaultHostLayout/);
  assert.match(host.get("src/atlas-bootstrap.ts"), /createBrowserRouter/);
  assert.doesNotMatch(host.get("src/atlas-bootstrap.ts"), /startHost|atlasConfig/);
  assert.match(host.get("src/HostAtlasProvider.tsx"), /import \{ AtlasHostProvider \} from "@atlas\/runtime\/react"/);
