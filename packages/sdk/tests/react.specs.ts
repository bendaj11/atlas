...
test("React generator emits React 19 Vite Native Federation projects", () => {
  ...
  assert.match(host.get("src/main.tsx"), /startAtlasHost/);
  assert.doesNotMatch(host.get("src/main.tsx"), /startHost|createBrowserRouter|atlasConfig/);
  assert.match(host.get("src/atlas-bootstrap.ts"), /AtlasDefaultHostLayout, startHost/);
  assert.match(host.get("src/atlas-bootstrap.ts"), /createBrowserRouter/);
  assert.match(host.get("src/atlas-bootstrap.ts"), /import atlasConfig from "\.\.\/atlas\.config"/);
  ...
  assert.match(host.get("src/atlas-bootstrap.ts"), /hostData: \{ hostId: atlasConfig\.id, name: atlasConfig\.name \}/);
  assert.doesNotMatch(host.get("src/atlas-bootstrap.ts"), /showToast|openModal|openPopup|createDomOverlayProviders/);
