...
test("React generator emits React 19 Vite Native Federation projects", () => {
  ...
  assert.match(host.get("src/HostAtlasProvider.tsx"), /from "@atlas\/sdk\/federation"/);
  assert.doesNotMatch(host.get("src/HostAtlasProvider.tsx"), /@softarc\/native-federation-runtime/);
  assert.match(host.get("src/HostAtlasProvider.tsx"), /export function HostAtlasProvider/);
  ...
  assert.match(host.get("src/HostAtlasProvider.tsx"), /import atlasConfig from "\.\.\/atlas\.config"/);
  ...
  assert.match(host.get("src/HostAtlasProvider.tsx"), /hostData: \{ hostId: atlasConfig\.id, name: atlasConfig\.name \}/);
  assert.doesNotMatch(host.get("src/HostAtlasProvider.tsx"), /showToast|openModal|openPopup|createDomOverlayProviders/);
  ...
  assert.match(host.get("server/main.mts"), /runAtlasHostServer/);
  assert.equal(JSON.parse(host.get("server/tsconfig.json")).compilerOptions.module, "NodeNext");
  assert.match(host.get("package.json"), /"@atlas\/host-server"/);
...
test("React generator targets selected supported majors with React Compiler", () => {
  ...
  assert.match(react19.get("package.json"), /"@vitejs\/plugin-react": "\^5\.0\.4"/);
  assert.doesNotMatch(react19.get("package.json"), /@rolldown\/plugin-babel|"rolldown"/);
