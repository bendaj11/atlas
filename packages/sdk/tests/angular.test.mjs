...
test("Angular generator emits Angular 20 Native Federation projects", () => {
  ...
  assert.match(host.get("src/bootstrap.ts"), /hostData: \{ hostId: atlasConfig\.id, name: atlasConfig\.name \?\? atlasConfig\.id \}/);
  ...
  assert.match(mf.get("src/app/starter/shell/starter-shell.component.ts"), /export class StarterShellComponent/);
  assert.match(mf.get("src/app/routes.ts"), /export const routes: Routes/);
  assert.match(mf.get("src/app/starter/shell/starter-shell.component.ts"), /router-outlet/);
  ...
  assert.match(mf.get("src/entry.ts"), /bootstrapApplication\(StarterShellComponent/);
