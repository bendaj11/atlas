...
import {
  HostRuntimeDriver,
  WidgetRegistryDriver,
  WidgetRetryDriver,
  createDeferred,
  createHostCatalog,
  createRouteManifest,
  createRoutePlacement,
  createSlotManifest,
  createTestContainer,
  createTestDocument,
  createTestElement,
  createWidgetRendererContainer,
  duplicateRegistryWidgetResult,
  duplicateWidgetResult,
  widgetProvider,
  widgetRegistry,
} from './loader.driver.js';
...
test('resolveRuntimeManifests applies overrides', () => {
  const production = createTestManifest({
    version: '1.0.0',
    channel: 'production',
  });
  ...
  const manifests = resolveRuntimeManifests(catalog, [
    { appId: 'catalog', manifest: local, reason: 'local' },
  ]);
...
test('resolveRuntimeCatalog applies widget provider overrides', () => {
  ...
  assert.equal(
    resolved.widgetProviders?.[0]?.remoteEntryUrl,
    replacement.remoteEntryUrl,
  );
});
  ...
  assert.throws(
    () =>
      resolveRuntimeManifests(catalog, [
        {
          appId: 'unknown',
          manifest: createTestManifest({ id: 'unknown' }),
          reason: 'local',
        },
      ]),
...
test('resolveRuntimeManifests preserves ambiguous exact routes for runtime conflict reporting', () => {
  const route = (id: string, path: string): AtlasPlacement[] => [
    createRoutePlacement(id, path),
  ];
  const catalog = createHostCatalog([
    createTestManifest({ id: 'first', placements: route('first', '/orders') }),
    createTestManifest({
      id: 'second',
      placements: route('second', '/orders/'),
    }),
  ]);

  assert.deepEqual(
    resolveRuntimeManifests(catalog).map((manifest) => manifest.id),
    ['first', 'second'],
  );
});
...
test('resolveRuntimeManifests preserves catalog placement ownership', () => {
  ...
  const selected = createTestManifest({
    id: 'catalog',
  ...
  const [resolved] = resolveRuntimeManifests(catalog, [
    { appId: 'catalog', manifest: replacement, reason: 'local' },
  ]);
...
test('createHostNavigationItems exposes resolved route metadata for custom host nav', () => {
  ...
  const navigation: AtlasNavigation = {
    ...
    getCurrentLocation() {
      return { pathname: '/orders/42', search: '', hash: '' };
    },
  };
  const manifests = [
    createTestManifest({
      ...
      placements: [
        {
          ...
          route: {
            path: '/hidden',
            title: 'Hidden',
            nav: { label: 'Hidden', visible: false },
          },
        },
      ],
    }),
...
test('createHostNavigationItems keeps the first app for duplicate exact routes', () => {
  const navigation: AtlasNavigation = {
    ...
    getCurrentLocation() {
      return { pathname: '/catalog', search: '', hash: '' };
    },
  };
  const route = (id: string, path: string): AtlasPlacement[] => [
    createRoutePlacement(id, path),
  ...
  const manifests = [
    ...
    createTestManifest({
      id: 'catalog',
      placements: route('catalog', '/catalog'),
    }),
  ];

  assert.deepEqual(
    createHostNavigationItems(manifests, 'host', navigation).map(
      (item) => item.appId,
    ),
    ['first', 'catalog'],
  );
...
test('resolveRuntimeManifests does not require consumed widgets in app manifests', () => {
  ...
  assert.equal(
    resolveRuntimeManifests(catalog, [
      { appId: 'orders', manifest: replacement, reason: 'local' },
    ]).length,
    2,
  );
});
  ...
  await assert.rejects(
    () =>
      loadHostCatalog({
        ...
        async fetchJson() {
          return { ...createHostCatalog([]), hostId: '' };
        },
      }),
...
test('resilient operations emit structured retry and success events', async () => {
  ...
  assert.deepEqual(
    ...
    ['operation.retry', 'operation.success'],
  );
  if (
    events[0]?.type !== 'operation.retry' ||
    events[1]?.type !== 'operation.success'
  ) {
  ...
  await assert.rejects(
    () =>
      runResiliently(
        ...
        {
          stage: 'remote-module',
          resource: 'https://cdn.example/entry.js',
          appId: 'map',
          version: '3.2.1',
        },
...
test('Native Federation keeps healthy remotes loadable when another initialization fails', async () => {
  ...
  assert.deepEqual(initialized, [['atlas_healthy'], ['atlas_broken']]);
...
test('host UI uses one host-owned outlet and supports custom loading and fallback renderers', () => {
  ...
  const ui = createHostUi({
    document,
    anchors,
    renderHostLoading(target) {
      states.push('loading');
      target.textContent = 'Preparing workspace';
      return () => {
        disposed = true;
      };
    },
    renderHostError(target, error, retryAction) {
      states.push(error.message);
      target.textContent = 'Custom fallback';
      retry = retryAction;
    },
  });
  ui.showLoading();
...
test('default host UI hides diagnostic details', () => {
  ...
  ui.showError(
    new Error(
      'Invalid Atlas host catalog. manifests.1.id: Duplicate app id "angular-app".',
    ),
    () => undefined,
  );
  ...
  assert.doesNotMatch(
    ...
    /Duplicate app id|Suggested action/,
  );
});
...
test('host placement state never removes a nested widget status', () => {
  ...
  widgetStatus.remove = () => {
    widgetStatusRemoved = true;
  };
  const container = createTestElement();
  container.setAttribute = () => undefined;
  container.querySelector = ((selector: string) =>
    selector === ':scope > [data-atlas-placement-status]'
      ? null
      : widgetStatus) as typeof container.querySelector;
  const document = createTestDocument();
      ...
      getItem(key) {
        return key === ATLAS_OVERRIDE_DOCUMENT_STORAGE_KEY
          ? JSON.stringify({
              ...
              generatedAt: 'now',
              overrides: [
                { appId: manifest.id, manifest, reason: 'historical' },
              ],
            })
          : null;
      },
...
test('tab-scoped browser overrides take precedence over all-tab overrides', async () => {
  ...
  const documentFor = (manifest: AtlasManifest) =>
    JSON.stringify({
      ...
      hostId: 'host',
      generatedAt: new Date().toISOString(),
      overrides: [{ appId: 'orders', manifest, reason: 'historical' }],
    });
  const overrides = await loadBrowserRuntimeOverrides({
    hostId: 'host',
    allowCustomOverrides: true,
...
test('widget loader mounts from the selected owner version', async () => {
  ...
  const mounted = await loader.mount(
...
test('widget loader uses host-owned loading UI and clears it after mount', async () => {
  ...
  const loader = createWidgetLoader([manifest], createTestHostSdk(), {
    ...
    renderWidgetLoading(container, context) {
      events.push(
        `loading:${context.widget?.name}:${context.ownerManifest?.id}`,
      );
      container.textContent = 'Custom widget loading';
      return () => {
        events.push('loading-disposed');
      };
    },
  });
...
test('widget getWidget loading renderer overrides host loading UI', async () => {
  ...
  const handle = loader.getWidget(widget.id, {
    renderLoading() {
      ...
      return () => {
        events.push('widget-loading-disposed');
      };
    },
  });
...
test('widget loader uses host-owned error UI and disposes it on unmount', async () => {
  ...
  assert.match(
    events[2] ?? '',
    /Atlas could not mount widget "unknown\/widget"/,
  );
...
test('widget registry resolves widgets from non-routed apps in the primary registry', async () => {
  ...
  assert.equal(
    (await resolver(widgetId)).ownerManifest.id,
    'internal-provider',
  );
  ...
  const resolver = createRegistryWidgetResolver({
    ...
    async fetchJson() {
      ...
      return widgetRegistry([
        widgetProvider('internal-provider', widgetId, '1.0.0'),
      ]);
    },
  });
...
test('widget registry resolves declared external providers at their production selection', async () => {
  ...
  const currentProvider = widgetProvider(
    'external-provider',
    widgetId,
    '2.0.0',
  );
  const consumer = createTestManifest({
    externalAppsDependencies: ['external-provider'],
  });
  const resolver = createRegistryWidgetResolver({
    runtimeConfig: {
      schemaVersion: '1',
    ...
    async fetchJson(url) {
      ...
      return widgetRegistry([oldProvider, currentProvider], {
        'external-provider': { version: '2.0.0', buildId: 'build-2.0.0' },
      });
    },
  });
...
test('widget registry retries after a transient registry failure', async () => {
  ...
  await assert.rejects(
    () => registry.resolver(registry.widgetId),
    /temporary registry failure/,
  );
  assert.equal(
    (await registry.resolver(registry.widgetId)).ownerManifest.id,
    'internal-provider',
  );
...
test('Angular component style assets resolve before the style enters the host document', () => {
  ...
  assert.equal(
    style.textContent,
    ".aaa[_ngcontent-orders-c0]{background:url('http://localhost:4202/assets/photo.png')}",
  );
});
...
test('document styles without asset URLs bypass rewriting', () => {
  ...
  assert.equal(
    style.textContent,
    '.aaa[_ngcontent-orders-c0]{color:rebeccapurple}',
  );
});
...
test('style elements inside the app boundary resolve remote assets', () => {
  ...
  assert.equal(
    style.textContent,
    ".aaa{background:url('http://localhost:4202/assets/photo.png')}",
  );
});
...
test('host runtime mounts only the active route and unmounts during navigation', async () => {
  ...
  const driver = new HostRuntimeDriver().given
    .manifests([
      createRouteManifest('catalog', '/catalog'),
      createRouteManifest('details', '/catalog/details'),
    ])
    .given.remote(async (manifest) => ({
      mount: () => ({
        unmount: () => {
          unmounted.push(manifest.id);
        },
      }),
    }));
...
test('host runtime lets the latest navigation supersede a pending mount', async () => {
  ...
  getAtlasNavigation(sdk).navigate('/slow');
  await tick();
  getAtlasNavigation(sdk).navigate('/latest');
  await tick();
...
test('host runtime renders the first exact route and logs duplicate routes', async () => {
  ...
  try {
    ...
    assert.match(
      String((errors[0]?.[1] as { message?: string }).message),
      /two apps assigned to the same host route/,
    );
    assert.deepEqual(
      (errors[0]?.[1] as { suggestedActions?: string[] }).suggestedActions,
      [
    ...
    getAtlasNavigation(sdk).navigate('/orders');
    await tick();
    getAtlasNavigation(sdk).navigate('/catalog');
    await tick();
...
test('host runtime mounts slots independently and reports remote failures', async () => {
  ...
  assert.equal(
    (driver.lastError as AtlasError | undefined)?.surface,
    'browser',
  );
...
test('DOM host mounts a deferred slot app when its anchor becomes available', async () => {
  ...
  try {
    ...
    const runtime = await startDomHostRuntime({
      ...
      services: {
        createNavigation: () => getAtlasNavigation(createTestHostSdk()),
      },
      document,
...
test('host runtime starts independent slots concurrently', async () => {
  ...
  let activeImports = 0;
  const driver = new HostRuntimeDriver().given
    .manifests([createSlotManifest('first'), createSlotManifest('second')])
    .given.remote(async () => {
      activeImports += 1;
...
test('host runtime cleans up mounts that finish after their timeout', async () => {
  const widget = createTestManifest({
    id: 'late',
    placements: [
      { id: 'late-slot', kind: 'slot', hostId: 'host', slot: 'late' },
    ],
  });
  let unmounted = 0;
  const runtime = await startAtlasHostRuntime({
...
test('host runtime coalesces overlapping retries for one failed placement', async () => {
  ...
  const driver = new HostRuntimeDriver().given
    .manifests([createSlotManifest('retry')])
...
test('host runtime shows loading UI only when requested by the app', async () => {
  const driver = new HostRuntimeDriver().given
    .manifests([createSlotManifest('widget', 'header')])
    .given.remote(async () => ({
      mount: ({ context }) => context.loading.show(),
    }));
...
test('host runtime renders no loading state when the app does not request one', async () => {
  const driver = new HostRuntimeDriver().given.manifests([
    createSlotManifest('widget', 'header'),
  ]);
...
function createAssetElement(
  ...
  Object.defineProperty(element, 'querySelectorAll', {
    value: () =>
      element.testChildren.flatMap((child) => [
        child,
        ...Array.from(child.querySelectorAll('*')),
      ]),
  });
  element.append = (...nodes) => {
    element.testChildren.push(...nodes.filter(isTestAssetElement));
  };
  element.prepend = (...nodes) => {
    element.testChildren.unshift(...nodes.filter(isTestAssetElement));
  };
  element.appendChild = (node) => {
    if (isTestAssetElement(node)) element.testChildren.push(node);
    return node;
  };
  ...
  element.replaceChild = (node, oldNode) => {
    if (!isTestAssetElement(node) || !isTestAssetElement(oldNode))
      return oldNode;
    const index = element.testChildren.indexOf(oldNode);
    if (index === -1) element.testChildren.push(node);
    else element.testChildren[index] = node;
    return oldNode;
  };
  element.replaceChildren = (...nodes) => {
    element.testChildren = nodes.filter(isTestAssetElement);
  };
  return element;
}
...
function isTestAssetElement(value: unknown): value is TestAssetElement {
  return (
    typeof value === 'object' &&
    value !== null &&
    'testChildren' in value &&
    Array.isArray(value.testChildren)
  );
}
  ...
  Object.defineProperty(document, 'createElement', {
    ...
    value: (tag: string) => {
      ...
      link.addEventListener = (
        ...
        if (
          (name === 'load' || name === 'error') &&
          typeof listener === 'function'
        )
          link.testListeners[name] = listener;
      };
      link.remove = () => {
        link.removed = true;
      };
...
function isTestStyleLink(value: unknown): value is TestStyleLink {
  return (
    typeof value === 'object' && value !== null && 'testListeners' in value
  );
}
...
test('remote entry integrity rejects modified production assets', async () => {
  ...
  await assert.rejects(
    () =>
      verifyManifestIntegrity(
        [manifest],
        async () => new TextEncoder().encode('changed').buffer,
      ),
...
test('production remotes allow optional integrity and require HTTP URLs', async () => {
  const missingIntegrity = createTestManifest({
    remoteEntryUrl: 'https://assets.example.com/remoteEntry.json',
  });
  await verifyManifestIntegrity(
    [missingIntegrity],
...
test('federation isolates rejected remotes while trusted apps remain loadable', async () => {
  const rejected = createTestManifest({
    id: 'rejected',
    remoteEntryUrl: 'https://assets.example.com/rejected.json',
  });
  const local = createTestManifest({
    id: 'local',
    channel: 'local',
  ...
  assert.deepEqual(initialized, []);
  assert.equal(
    typeof (await federation.importRemote(rejected)).mount,
    'function',
  );
...
test('federation trusts a registry-resolved widget owner manifest', async () => {
  ...
  assert.equal(
    typeof (await federation.importWidget(widget, owner)).mount,
    'function',
  );
