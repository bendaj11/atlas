      ...
      () =>
        runtime
          .initFederation(
            ...
            hostRemoteEntryUrl
              ? { deployUrl: artifactDirectoryUrl(hostRemoteEntryUrl) }
              : undefined,
          )
          .then(() => undefined),
      {
  ...
  return {
    async initialize(manifests) {
      await mapWithConcurrency(manifests, async (manifest) => {
        try {
          await initializeRemote(manifest);
        } catch {
          return;
        }
      });
    ...
    async importRemote(manifest) {
      ...
      const entry = await runResiliently(
        () =>
          runtime.loadRemoteModule(
            requireInitializedRemoteName(remoteNames, manifest.id),
    ...
    async importWidget(widget, ownerManifest) {
      ...
      const entry = await runResiliently(
        () =>
          runtime.loadRemoteModule(
            requireInitializedRemoteName(remoteNames, widget.ownerAppId),
            widget.expose,
          ),
        {
...
export async function createTrustedNativeFederationImporters(
  ...
  return {
    ...
    async importWidget(widget, ownerManifest) {
      ...
      if (!manifest) {
        throw runtimeError(
          `Atlas cannot load widget "${widget.id}" because owner app "${widget.ownerAppId}" is not trusted by this host.`,
          {
            suggestedActions:
              'Add the owner app manifest to the selected host catalog, then republish the catalog and reload.',
      ...
      if (manifest.id !== widget.ownerAppId) {
        throw runtimeError(
          `Atlas cannot load widget "${widget.id}" because its owner manifest does not match app "${widget.ownerAppId}".`,
          {
            suggestedActions:
...
async function fetchRemoteBytes(
  ...
  if (!response.ok) {
    throw runtimeError(
      ...
      {
        suggestedActions:
          'Deploy the remote entry at the manifest URL and verify the host can fetch it through CORS, then retry.',
...
function normalizeAppEntry(value: unknown, id: string): AtlasAppEntry {
  ...
  if (!hasMountFunction(entry)) {
    throw runtimeError(
      `Atlas cannot mount app "${id}" because its remote module does not export mount(request).`,
      {
        suggestedActions:
...
function normalizeWidgetEntry(
  ...
  if (!hasMountFunction(entry)) {
    throw runtimeError(
      `Atlas cannot mount exported widget "${ref}" because its remote module does not export mount(request).`,
      {
        suggestedActions:
...
function hasMountFunction(
  value: unknown,
): value is AtlasAppEntry & AtlasExportedWidgetEntry {
  return (
    typeof value === 'object' &&
    value !== null &&
    'mount' in value &&
...
function requireInitializedRemoteName(
  ...
  if (!remoteName) {
    throw runtimeError(
      `Atlas cannot load app "${appId}" because Native Federation was not initialized for it.`,
      {
        suggestedActions:
          'Verify the app manifest remoteEntryUrl and federation initialization failure, then retry loading the app.',
