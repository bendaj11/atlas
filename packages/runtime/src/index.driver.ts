...
interface TestProductionSelection {
  version: string;
  buildId: string;
}
import type {
...
function testHostManifest(id: string): AtlasHostManifest {
  return {
    ...
    channel: 'production',
    framework: 'react',
    remoteEntryUrl: 'https://cdn.example/host/remoteEntry.json',
    exposes: { entry: './host' },
...
export function createTestContainer(
  ...
  element.attachShadow = () =>
    createTestContainer(document) as unknown as ShadowRoot;
  return element;
}
...
export function createWidgetRendererContainer(): HTMLElement {
  ...
  const createElement = (): HTMLElement => {
    ...
    Object.defineProperty(element, 'isConnected', {
      get: () => connected.get(element) ?? false,
    });
    Object.defineProperty(element, 'ownerDocument', { value: document });
    element.append = (...nodes) => {
      for (const node of nodes) {
        if (typeof node !== 'object' || node === null || !('dataset' in node))
          continue;
...
export class WidgetRetryDriver {
  ...
  constructor() {
    this.#remoteMounted = new Promise((resolve) => {
      this.#resolveRemoteMounted = resolve;
    });
  }
  ...
  async start(): Promise<void> {
    ...
    const loader = createWidgetLoader([manifest], createTestHostSdk(), {
      ...
      renderWidgetError: (_container, _context, retry) => {
        this.#retry = retry;
      },
    });
    this.#mounted = await loader.mount(
      widget.id,
...
export async function duplicateWidgetResult(): Promise<{
  ...
  const first = createTestManifest({
    id: 'first',
    exportedWidgets: [widgetManifest('first', widgetId, 'First Widget')],
  });
  const second = createTestManifest({
    id: 'second',
    exportedWidgets: [widgetManifest('second', widgetId, 'Second Widget')],
  });
  ...
  try {
    ...
    const widget = await createWidgetLoader(
      [first, second],
      createTestHostSdk(),
    ).getWidget(widgetId);
...
export function widgetRegistry(
  apps: AtlasManifest[],
  selections: Record<string, TestProductionSelection> = {},
): AtlasStaticRegistry {
  return {
    schemaVersion: '1',
  ...
  } as unknown as AtlasStaticRegistry;
}
...
export function createRouteManifest(id: string, path: string): AtlasManifest {
  return createTestManifest({
    id,
    placements: [
      { id: `${id}-route`, kind: 'route', hostId, route: { path, title: id } },
    ],
  });
}
...
export class HostRuntimeDriver {
  ...
  #stateWaiters: Array<{
    matches: (event: AtlasHostMountEvent) => boolean;
    resolve: (event: AtlasHostMountEvent) => void;
  }> = [];
  ...
  when = {
    ...
    navigatedTo: async (
      path: string,
      expectedState: AtlasHostMountState = 'mounted',
    ) => {
  ...
  #recordState(event: AtlasHostMountEvent): void {
    ...
    const matchingWaiters = this.#stateWaiters.filter(({ matches }) =>
      matches(event),
    );
  ...
  #waitForState(
    matches: (event: AtlasHostMountEvent) => boolean,
  ): Promise<AtlasHostMountEvent> {
    return new Promise((resolve) =>
      this.#stateWaiters.push({ matches, resolve }),
    );
  }
}
