...
export class AtlasLoadError extends AtlasError {
  ...
  constructor(
    ...
    const details = [
      ...
      .join(', ');
    super(
      `Atlas could not load a required resource (${details}): ${errorSummary(errorMessage(cause))}`,
      {
...
function actionForStage(stage: string): string {
  if (stage === 'catalog')
    return 'Verify catalog URL, response status, JSON schema, and network access, then retry.';
...
function validatePolicy(policy: {
  ...
  if (!Number.isInteger(policy.timeoutMs) || policy.timeoutMs < 1) {
    throw runtimeError(
      `Atlas request timeoutMs must be a positive integer; received ${policy.timeoutMs}.`,
      {
        suggestedActions:
  ...
  if (!Number.isInteger(policy.retryCount) || policy.retryCount < 0) {
    throw runtimeError(
      `Atlas retryCount must be a non-negative integer; received ${policy.retryCount}.`,
      {
        suggestedActions:
    ...
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => {
          ...
          reject(
            new Error(
              `Timed out after ${timeoutMs}ms during ${context.stage}.`,
            ),
          );
        }, timeoutMs);
      }),
