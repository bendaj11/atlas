...
test('atlas generation registers projects with Nx automatically', async () => {
  ...
  expect(stdout).toMatch(
    /Native Nx scaffolding was skipped; Atlas will generate the React scaffold directly at orders/,
  );
});
...
for (const scenario of [
  ...
  test(`atlas generates complete app files in a ${scenario.name}`, async () => {
    ...
    expect(await readFile(join(projectRoot, scenario.entry), 'utf8')).toMatch(
      scenario.match,
    );
    expect(await readFile(join(projectRoot, scenario.config), 'utf8')).toMatch(
      scenario.configMatch,
    );
    expect(
      await readFile(join(projectRoot, 'atlas.config.ts'), 'utf8'),
    ).toMatch(new RegExp(`framework: "${scenario.framework}"`));
    expect(
    ...
    expect(stdout).toMatch(
      new RegExp(
        `Detected ${scenario.name === 'Turborepo' ? 'a Turborepo' : 'a package-manager'} workspace`,
      ),
    );
  });
}
...
for (const scenario of [
  ...
  test(`atlas generates host workspace scripts in a ${scenario.name}`, async () => {
    ...
    expect(packageJson.scripts['atlas:publish']).toBe(
      'atlas publish customer-host --from-build-output',
    );
    expect(packageJson.scripts['atlas:bootstrap']).toBe(
    ...
    if (scenario.name === 'Turborepo') {
      ...
      expect(turbo.tasks['atlas:publish'].env).toEqual(
        expect.arrayContaining(['ATLAS_*', 'AWS_*', 'CI_*']),
      );
      expect(turbo.tasks['atlas:bootstrap'].outputs).toStrictEqual([
        'dist/bootstrap/**',
      ]);
    }
    if (scenario.framework === 'angular') {
      const appTsconfig = JSON.parse(
        await readFile(join(root, scenario.root, 'tsconfig.app.json'), 'utf8'),
      );
      const angularJson = JSON.parse(
        await readFile(join(root, scenario.root, 'angular.json'), 'utf8'),
      );
      ...
      const rootTsconfig = JSON.parse(
        await readFile(join(root, scenario.root, 'tsconfig.json'), 'utf8'),
      );
      expect(rootTsconfig.compilerOptions.target).toBe('ES2022');
      await expect(
        access(join(root, scenario.root, 'tsconfig.atlas.json')),
