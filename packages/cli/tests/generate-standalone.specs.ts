...
test('atlas generates a portable Angular host at an explicit directory', async () => {
  ...
  const federationConfig = await readFile(
    join(target, 'federation.config.mjs'),
    'utf8',
  );
  expect(await readFile(join(target, 'atlas.bootstrap.html'), 'utf8')).toMatch(
    /id="atlas-host-root">Loading product…<\/div>/,
  );
  await expect(
    access(join(target, 'public/atlas.runtime.json')),
  ).rejects.toMatchObject({ code: 'ENOENT' });
  expect(await readFile(join(target, 'atlas.config.ts'), 'utf8')).not.toMatch(
    /resourcesTimeoutMs/,
  );
  await expect(access(join(target, 'Containerfile'))).rejects.toMatchObject({
    code: 'ENOENT',
  });
  await expect(access(join(target, 'server'))).rejects.toMatchObject({
    code: 'ENOENT',
  });
  expect(await readFile(join(target, 'atlas.config.ts'), 'utf8')).not.toMatch(
    /catalogUrl/,
  ...
  expect(generatedPackage.scripts.dev).toBe('atlas dev customer-host');
  expect(generatedPackage.scripts['framework:dev']).toBe(
    'ng serve customer-host',
  );
  expect(generatedPackage.devDependencies['@atlas/cli']).toBeDefined();
  expect(generatedPackage.scripts.build).toBe('ng build');
  expect(generatedPackage.scripts['atlas:publish']).toBe(
    'atlas publish customer-host --from-build-output',
  );
  expect(generatedPackage.scripts['atlas:bootstrap']).toBe(
    'atlas build-bootstrap customer-host --skip-compile',
  );
  ...
  const angularJson = JSON.parse(
    await readFile(join(target, 'angular.json'), 'utf8'),
  );
  ...
  expect(main).toMatch(/catch \(cause: unknown\)/);
  expect(federationConfig).toMatch(/createAngularV4FederationConfig/);
  expect(bootstrap).toMatch(/bootstrapAngularHost/);
  expect(bootstrap).toMatch(/export const mount: AtlasHostClientEntry/);
  expect(bootstrap).not.toMatch(/localhost:4300/);
  await expect(
    access(join(target, 'src/app/atlas-host-default-route.component.ts')),
  ).rejects.toMatchObject({ code: 'ENOENT' });
  await expect(access(join(target, 'src/host.ts'))).rejects.toMatchObject({
    code: 'ENOENT',
  });
  expect(await readFile(join(target, 'src/app/app.routes.ts'), 'utf8')).toMatch(
    /AtlasDefaultHostRouteComponent/,
  );
  expect(await readFile(join(target, 'src/app/app.config.ts'), 'utf8')).toMatch(
    /provideZonelessChangeDetection/,
  );
});

test('atlas generates Angular projects with requested stylesheet format', async () => {
  const temporary = await mkdtemp(join(tmpdir(), 'atlas-angular-stylesheet-'));
  const target = join(temporary, 'customer-host');
  ...
  expect(
    angularJson.projects['customer-host'].architect.esbuild.options.styles,
  ).toStrictEqual(['src/styles.scss']);
  await access(join(target, 'src/styles.scss'));
  await expect(access(join(target, 'src/styles.css'))).rejects.toMatchObject({
    code: 'ENOENT',
  });
});
...
test('atlas app generation only writes a route when a host is supplied', async () => {
  ...
  const defaultConfig = await readFile(
    join(withoutHost, 'atlas.config.ts'),
    'utf8',
  );
  ...
  const explicitConfig = await readFile(
    join(withHost, 'atlas.config.ts'),
    'utf8',
  );
  expect(await readFile(join(withHost, 'vite.config.ts'), 'utf8')).toMatch(
    /server: \{ port: 4306, cors: true \}/,
  );
...
test('atlas app generation can create single-page apps without inner route files', async () => {
  const temporary = await mkdtemp(
    join(tmpdir(), 'atlas-single-page-generator-'),
  );
  ...
  expect(await readFile(join(reactRoot, 'src/bootstrap.tsx'), 'utf8')).toMatch(
    /defineApp/,
  );
  expect(
    await readFile(join(reactRoot, 'src/bootstrap.tsx'), 'utf8'),
  ).not.toMatch(/createRoutedApp|RouterProvider/);
  expect(await readFile(join(reactRoot, 'src/App.tsx'), 'utf8')).toMatch(
    /Single-page Atlas app/,
  );
  await expect(access(join(reactRoot, 'src/routes.tsx'))).rejects.toMatchObject(
    { code: 'ENOENT' },
  );
  await expect(
    access(join(reactRoot, 'src/home/Home.tsx')),
  ).rejects.toMatchObject({ code: 'ENOENT' });
  await expect(
    access(join(reactRoot, 'src/details/Details.tsx')),
  ).rejects.toMatchObject({ code: 'ENOENT' });

  const angularPackage = JSON.parse(
    await readFile(join(angularRoot, 'package.json'), 'utf8'),
  );
  expect(angularPackage.dependencies['@angular/router']).toBe(undefined);
  expect(await readFile(join(angularRoot, 'src/entry.ts'), 'utf8')).toMatch(
    /defineApp/,
  );
  expect(
    await readFile(join(angularRoot, 'src/app/app.config.ts'), 'utf8'),
  ).not.toMatch(/provideRouter|LocationStrategy/);
  expect(
    await readFile(join(angularRoot, 'src/app/app.component.ts'), 'utf8'),
  ).toMatch(/Single-page Atlas app/);
  await expect(
    access(join(angularRoot, 'src/app/app.routes.ts')),
  ).rejects.toMatchObject({ code: 'ENOENT' });
  await expect(
    access(join(angularRoot, 'src/app/home/home.component.ts')),
  ).rejects.toMatchObject({ code: 'ENOENT' });
  await expect(
    access(join(angularRoot, 'src/app/details/details.component.ts')),
  ).rejects.toMatchObject({ code: 'ENOENT' });
});
...
test('atlas build identifies and versions a host client', async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: 'acme', private: true, workspaces: ['host'] }),
  );
  await writeFile(
    join(projectRoot, 'package.json'),
    JSON.stringify({ name: 'host', version: '0.1.0', type: 'module' }),
  );
  ...
  await writeFile(
    join(projectRoot, 'dist/remoteEntry.json'),
    JSON.stringify({
      name: 'atlas_host',
  ...
  const manifest = JSON.parse(
    await readFile(join(projectRoot, 'dist/host.manifest.json'), 'utf8'),
  );
  expect(manifest.kind).toBe('host');
  expect(manifest.version).toBe('0.1.0');
  expect(
    manifest.remoteEntryUrl.startsWith(
      'https://cdn.example/atlas/hosts/host/0.1.0/',
    ),
  ).toBe(true);
  await expect(
    access(join(projectRoot, 'dist/atlas-publication.json')),
  ).rejects.toMatchObject({ code: 'ENOENT' });
});
...
test('atlas build-bootstrap emits deployable static files', async () => {
  ...
  await writeFile(
    join(projectRoot, 'package.json'),
    JSON.stringify({ name: 'host', version: '2.1.0', type: 'module' }),
  );
  await writeFile(join(projectRoot, 'atlas.config.ts'), 'export default {};\n');
  await writeFile(
    join(projectRoot, 'atlas.bootstrap.html'),
    '<main id="atlas-host-root">Project loading UI</main><script type="module" src="/atlas.loader.js"></script>\n',
  );
  await writeFile(
    join(projectRoot, 'other.bootstrap.html'),
    '<main id="atlas-host-root">Explicit loading UI</main><script type="module" src="/atlas.loader.js"></script>\n',
  );
  await writeFile(
    join(projectRoot, '.atlas/atlas.config.js'),
    `export default {
  ...
  const runtime = JSON.parse(
    await readFile(join(output, 'atlas.runtime.json'), 'utf8'),
  );
  expect(runtime.hostId).toBe('customer-host');
  expect(runtime.hostVersion).toBe('2.1.0');
  expect(runtime.catalogUrl).toBe(
    'https://cdn.example/atlas/hosts/customer-host/catalog.json',
  );
  expect(await readFile(join(output, 'index.html'), 'utf8')).toMatch(
    /Project loading UI/,
  );
  expect(await readFile(join(output, 'atlas.loader.js'), 'utf8')).toMatch(
    /requiredLoaderApiVersion/,
  );
  expect(await readFile(join(output, 'nginx.conf'), 'utf8')).toMatch(
  ...
  expect(await readFile(join(explicitOutput, 'index.html'), 'utf8')).toMatch(
    /Explicit loading UI/,
  );
});
...
test('atlas compile-config emits atlas.config.js through the project tsconfig', async () => {
  ...
  await writeFile(
    join(root, 'package.json'),
    JSON.stringify({ name: 'orders', version: '0.1.0', type: 'module' }),
  );
  await writeFile(
