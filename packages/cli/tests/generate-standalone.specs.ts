...
test("atlas generates a portable Angular host at an explicit directory", async () => {
  ...
  await run(process.execPath, ["packages/cli/dist/cli/entrypoint.js", "g", "host", "customer-host", "--framework=angular", "--port=4305", "--skip-install", `--directory=${target}`]);
  ...
  expect(generatedPackage.scripts["framework:dev"]).toBe("ng serve customer-host");
...
test("atlas generates Angular projects with requested stylesheet format", async () => {
  ...
  await run(process.execPath, [
    "packages/cli/dist/cli/entrypoint.js", "g", "host", "customer-host",
    "--framework=angular", "--style=scss", "--skip-install", `--directory=${target}`
  ]);

  const angularJson = JSON.parse(await readFile(join(target, "angular.json"), "utf8"));
  expect(angularJson.projects["customer-host"].architect.esbuild.options.styles).toStrictEqual(["src/styles.scss"]);
  await access(join(target, "src/styles.scss"));
  await expect(access(join(target, "src/styles.css"))).rejects.toMatchObject({ code: "ENOENT" });
});
...
test("atlas app generation only writes a route when a host is supplied", async () => {
  ...
  await run(process.execPath, [
    "packages/cli/dist/cli/entrypoint.js", "g", "app", "orders",
    "--framework=react", "--skip-install", `--directory=${withoutHost}`
  ]);
  await run(process.execPath, [
    "packages/cli/dist/cli/entrypoint.js", "g", "app", "billing",
...
test("atlas build identifies and versions a host client", async () => {
  ...
  await run(process.execPath, [
    join(process.cwd(), "packages/cli/dist/cli/entrypoint.js"), "build", "host",
...
test("atlas build-bootstrap emits deployable static files", async () => {
  ...
  await run(process.execPath, [
    join(process.cwd(), "packages/cli/dist/cli/entrypoint.js"), "build-bootstrap", "host",
    "--skip-compile", "--registry-base-url=https://cdn.example/atlas"
  ], { cwd: root });
...
test("atlas compile-config emits atlas.config.js through the project tsconfig", async () => {
  ...
  await run(process.execPath, [join(process.cwd(), "packages/cli/dist/cli/entrypoint.js"), "compile-config"], { cwd: root });
...
test("atlas removes a newly generated project when dependency installation fails", async () => {
  ...
  await expect(run(process.execPath, [
    join(process.cwd(), "packages/cli/dist/cli/entrypoint.js"), "g", "host", "customer-host",
