...
test('macOS browser opener always opens the URL', () => {
  expect(
    browserOpenCommand('http://localhost:5173/orders', 'darwin'),
  ).toStrictEqual({
    command: 'open',
...
test('local runtime configuration uses localhost', () => {
  ...
  expect(runtime.catalogUrl).toBe(
    'http://localhost:4400/hosts/customer-host/catalog.json',
  );
});

test('framework dev servers receive compatible localhost arguments', () => {
  expect(frameworkServerArguments('react', 4200)).toStrictEqual([
    '--port',
    '4200',
    '--host',
    'localhost',
  ]);
  expect(frameworkServerArguments('angular', 4201)).toStrictEqual([
    '--port',
    '4201',
  ]);
});
...
test('React app Vite config delegates Atlas-owned behavior', () => {
  const viteConfig = generateAppFiles({
    name: 'orders',
    framework: 'react',
  }).find(({ path }) => path === 'vite.config.ts')?.contents;

  expect(viteConfig).toMatch(/createReactAppViteConfig/);
  expect(viteConfig).not.toMatch(
    /atlas-react-source-reload|full-reload|remoteEntry\.json|rollupOptions/,
  );
});

test('Nx Angular federation repair writes cwd-independent expose paths', async () => {
  const root = await mkdtemp(
    join(tmpdir(), 'atlas-nx-angular-federation-repair-'),
  );
  const appRoot = join(root, 'apps/login');
  await mkdir(appRoot, { recursive: true });
  await writeFile(
    join(appRoot, 'federation.config.js'),
...
`,
  ...
  expect(
    [...source.matchAll(/join\(__dirname, "src\/entry\.ts"\)/g)].length,
  ).toBe(2);
  expect(
    [
      ...source.matchAll(
