...
`, { mode: 0o755 });
  ...
  expect(await readFile(join(root, "products/host-server/main.mts"), "utf8")).toMatch(/runAtlasHostServer/);
  ...
  expect(rootPackage.dependencies["@atlas/host-server"]).toBe(ATLAS_PACKAGE_RANGE);
...
`, { mode: 0o755 });
  ...
  expect(serverProject.targets.start.options.command).toBe("node dist/main.mjs");
  expect(await readFile(join(root, "apps/host-server/main.mts"), "utf8")).toMatch(/runAtlasHostServer/);
