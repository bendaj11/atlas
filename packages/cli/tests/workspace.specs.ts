...
test('workspace detection discovers an Nx project without consumer configuration', async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: '@acme/orders', version: '1.2.3' }),
  );
  await writeFile(
  ...
  expect(project.outputPaths).toStrictEqual([
    join(root, 'dist', 'apps', 'orders'),
  ]);
  expect(workspace.generationRoot('app', 'catalog')).toBe(
    join(root, 'apps', 'catalog'),
  );
});
...
test('package-less Nx projects inherit existing workspace release version', async () => {
  ...
  await writeFile(
    join(root, 'package.json'),
    JSON.stringify({ name: 'acme', version: '3.2.1' }),
  );
  await writeFile(
    join(projectRoot, 'project.json'),
    JSON.stringify({ name: 'orders', targets: {} }),
  );
  await writeFile(join(projectRoot, 'atlas.config.ts'), 'export default {};\n');

  const project = await (
    await detectWorkspace(projectRoot)
  ).findProject('orders');
...
test('Nx discovery identifies a project with missing Atlas configuration', async () => {
  const root = await createWorkspaceFixture('atlas-nx-missing-config-', {
    ...
    'apps/react-host/project.json': JSON.stringify({
      name: 'react-host',
      targets: {},
    }),
  });
...
test('Nx generation respects a direct child working directory', async () => {
  ...
  expect(workspace.generationRoot('app', 'orders')).toBe(
    join(appsRoot, 'orders'),
  );
});
...
test('pnpm-workspace.yaml establishes a pnpm workspace without package.json workspaces', async () => {
  ...
  await writeFile(
    join(root, 'package.json'),
    JSON.stringify({ name: 'acme', packageManager: 'pnpm@10.0.0' }),
  );
  await writeFile(
    join(projectRoot, 'package.json'),
    JSON.stringify({ name: 'orders', version: '1.0.0' }),
  ...
  expect(workspace.generationRoot('host', 'host')).toBe(
    join(root, 'packages', 'host'),
  );
});
...
test('Nx discovery includes configured and declared output directories', async () => {
  ...
  await writeFile(
    join(projectRoot, 'package.json'),
    JSON.stringify({ name: '@acme/orders', version: '1.0.0' }),
  );
  await writeFile(
    join(projectRoot, 'project.json'),
    JSON.stringify({
      ...
      targets: {
        build: {
          ...
          configurations: {
            development: { outputPath: 'dist/dev/orders' },
            production: {
              outputPath: { base: 'dist/orders', browser: 'browser' },
            },
          },
        },
      },
  ...
  expect(project.outputPaths).toStrictEqual([
    join(root, 'dist/orders/browser'),
    join(root, 'dist/orders'),
    join(root, 'dist/dev/orders'),
    join(root, 'custom/orders'),
...
test('Nx discovery follows Angular native federation delegated build output', async () => {
  const root = await mkdtemp(
    join(tmpdir(), 'atlas-nx-angular-native-outputs-'),
  );
  ...
  await writeFile(
    join(projectRoot, 'package.json'),
    JSON.stringify({ name: '@acme/mobile-host', version: '1.0.0' }),
  );
  await writeFile(
    join(projectRoot, 'project.json'),
    JSON.stringify({
      ...
      targets: {
        ...
        esbuild: {
          ...
          options: {
            outputPath: { base: 'apps/mobile-host/dist', browser: 'browser' },
          },
        },
      },
    }),
  ...
  const project = await (
    await detectWorkspace(projectRoot)
  ).findProject('mobile-host');

  expect(project.outputPaths).toStrictEqual([
    join(root, 'apps/mobile-host/dist/browser'),
    join(root, 'apps/mobile-host/dist'),
  ]);
});
...
test('task commands follow Nx, Turbo, and package-manager conventions', () => {
  const project = {
    id: 'orders',
    root: '/repo/apps/orders',
    packageName: '@acme/orders',
    version: '1.0.0',
    outputPaths: [],
  };
  expect(
    createTaskCommand('nx', 'yarn', '/repo', project, 'build'),
  ).toStrictEqual({
    command: 'yarn',
    args: ['nx', 'run', 'orders:build'],
    cwd: '/repo',
  });
  expect(
    createTaskCommand('turbo', 'pnpm', '/repo', project, 'dev', [
      '--port',
      '4201',
    ]),
  ).toStrictEqual({
  ...
  expect(
    createTaskCommand('turbo', 'yarn', '/repo', project, 'dev', [
      '--port',
      '4201',
    ]),
  ).toStrictEqual({
  ...
  expect(
    createTaskCommand('turbo', 'yarn', '/repo', project, 'atlas:config'),
  ).toStrictEqual({
    command: 'yarn',
    args: ['workspace', '@acme/orders', 'run', 'atlas:config'],
    cwd: '/repo',
  });
  expect(
    createTaskCommand('workspace', 'npm', '/repo', project, 'build'),
  ).toStrictEqual({
    command: 'npm',
    args: ['run', 'build', '--workspace', '@acme/orders'],
    cwd: '/repo',
  });
  expect(
    createTaskCommand('workspace', 'pnpm', '/repo', project, 'dev', [
      '--port',
      '4201',
    ]),
  ).toStrictEqual({
    command: 'pnpm',
    args: ['--filter', '@acme/orders', 'run', 'dev', '--port', '4201'],
    cwd: '/repo',
  });
  expect(
    createTaskCommand('workspace', 'pnpm', '/repo', project, 'framework:dev', [
      '--port',
      '4201',
    ]),
  ).toStrictEqual({
    command: 'pnpm',
    args: ['--filter', '@acme/orders', 'run', 'framework:dev', '--port', '4201'],
    cwd: '/repo',
  });
  expect(
    createTaskCommand('workspace', 'yarn', '/repo', project, 'dev', [
      '--port',
      '4201',
    ]),
  ).toStrictEqual({
    command: 'yarn',
    args: ['workspace', '@acme/orders', 'run', 'dev', '--port', '4201'],
    cwd: '/repo',
  });
});
...
test('dependency installs use the detected package manager from the generated project', async () => {
  expect(
    createInstallCommand('npm', '/repo', '/repo/apps/orders'),
  ).toStrictEqual({
    command: 'npm',
    args: ['install'],
    cwd: '/repo/apps/orders',
  });
  expect(
    createInstallCommand('pnpm', '/repo', '/repo/apps/orders'),
  ).toStrictEqual({
    command: 'pnpm',
    args: ['install'],
    cwd: '/repo/apps/orders',
  ...
  await writeFile(
    join(projectRoot, 'package.json'),
    JSON.stringify({ name: '@acme/orders' }),
  );
...
test('generated project formatting uses existing workspace tooling', async () => {
  ...
  await writeFile(
    join(nxRoot, 'package.json'),
    JSON.stringify({ devDependencies: { nx: '22.0.0' } }),
  );
  expect(
    await createFormatGeneratedCommand(
      ...
      'yarn',
      nxRoot,
      join(nxRoot, 'apps', 'orders'),
    ),
  ).toStrictEqual({
    command: 'yarn',
    args: ['nx', 'format:write', 'apps/orders'],
    cwd: nxRoot,
  ...
  expect(
    await createFormatGeneratedCommand('workspace', 'pnpm', root, projectRoot),
  ).toStrictEqual({
    command: 'pnpm',
    args: ['run', 'format'],
    cwd: projectRoot,
  ...
  expect(
    await createFormatGeneratedCommand('workspace', 'npm', root, projectRoot),
  ).toStrictEqual({
    command: 'npm',
    args: ['run', 'lint', '--', '--fix'],
    cwd: projectRoot,
...
test('non-interactive Nx projects use deterministic framework generator defaults', () => {
  expect(
    createNxGenerationCommand('pnpm', '/repo', {
      framework: 'angular',
      type: 'host',
      directory: 'apps/host',
      interactive: false,
  ...
  expect(
    createNxGenerationCommand('yarn', '/repo', {
      framework: 'react',
      type: 'app',
...
test('interactive Nx projects keep native generator defaults deterministic', () => {
  expect(
    createNxGenerationCommand('yarn', '/repo', {
      framework: 'angular',
      type: 'host',
...
test('solution-style Nx workspaces still require the native Angular plugin', async () => {
  ...
  const workspace = await detectWorkspace(root);
  expect(await workspace.missingScaffoldDependency('angular')).toBe(
    '@nx/angular',
  );
  expect(workspace.generationRoot('host', 'host')).toBe(
    join(root, 'packages', 'host'),
