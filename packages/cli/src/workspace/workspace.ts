...
import { ATLAS_NX_TAG } from '../generation/nx.js';
import {
  runProcess,
  spawnProcess,
  type ProcessCommand,
...
export async function detectWorkspace(
  ...
  return {
    ...
    run: (project, task, args = []) =>
      runProcess(
        createTaskCommand(kind, packageManager, root, project, task, args),
      ),
    ...
    installScaffoldDependency: async (projectType) => {
      if (kind === 'nx')
        await runProcess(
          createNxPluginInstallCommand(packageManager, root, projectType),
        );
    },
    scaffoldProject: async (options) => {
      ...
      try {
        await runProcess(
          createNxGenerationCommand(packageManager, root, {
        ...
        const plugin =
          options.framework === 'angular' ? '@nx/angular' : '@nx/react';
        throw new Error(
          `Nx could not scaffold "${options.name}". Install ${plugin} in the workspace and try again.`,
          { cause: error },
        );
      }
      return true;
...
export function createNxGenerationCommand(
  ...
  const args = [
    ...
    `--port=${port}`,
    ...(options.framework === 'angular'
      ? ['--ssr=false', `--style=${options.stylesheetFormat ?? 'css'}`]
      : []),
    '--e2eTestRunner=none',
    '--unitTestRunner=none',
...
export async function installationRoot(
  kind: AtlasWorkspaceKind,
  workspaceRoot: string,
  projectRoot: string,
): Promise<string> {
...
async function findWorkspaceRoot(start: string): Promise<string> {
  ...
  while (true) {
    if (
      ...
      (await declaresWorkspaces(current))
    )
      return current;
...
async function findAtlasProject(
  root: string,
  name: string,
...
async function discoverAtlasProjects(
  directory: string,
  workspaceRoot: string,
  depth: number,
): Promise<AtlasProject[]> {
...
async function readProject(
  ...
  if (!identifiers.includes(requestedName) && resolve(requestedName) !== root)
    return undefined;
...
function delegatedNxBuildTarget(
  ...
  if (build.executor !== '@angular-architects/native-federation:build')
    return undefined;
...
function configuredNxOutputPaths(
...
function declaredNxOutputPaths(
  target: NxTargetConfiguration,
  context: {
  ...
  return (target.outputs ?? [])
    .map((output) =>
      interpolateNxOutput(output, context.projectName, context.projectRoot),
    )
...
export function createTaskCommand(
  ...
  if (kind === 'turbo' && task !== 'atlas:config')
...
async function detectGenerationBase(
  root: string,
  start: string,
): Promise<string> {
...
async function packageIsInstalled(
  root: string,
  packageName: string,
): Promise<boolean> {
  if (
...
async function hasAny(root: string, names: string[]): Promise<boolean> {
  return (
    await Promise.all(names.map((name) => exists(join(root, name))))
  ).some(Boolean);
}
