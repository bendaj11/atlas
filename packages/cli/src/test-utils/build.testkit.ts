...
export function createTestWorkspace(
  ...
  return {
    ...
    async formatGenerated() {
      return false;
    },
    async installDependencies() {},
    async missingScaffoldDependency() {
      return undefined;
    },
...
export async function atlasPackageRange(): Promise<string> {
  const packageJson = JSON.parse(
    await readFile(
      new URL('../../../generators/package.json', import.meta.url),
      'utf8',
    ),
  );
...
export async function runDevService(
  ...
  try {
    ...
    await new AtlasDevService(
      workspace,
      args,
      new AtlasBuildService(workspace, args),
    ).run('orders', prompts);
    return stdout;
  } finally {
...
export function listenWithRemoteEntry(): Promise<{
  server: Server;
  port: number;
}> {
...
export async function catalogManifestIds(
  port: number,
  hostId: string,
): Promise<string[]> {
...
export async function devSessionHostId(
  port: number,
  hostId: string,
): Promise<string> {
  const response = await fetch(
    `http://localhost:${port}/atlas.dev-session.json?hostId=${encodeURIComponent(hostId)}`,
    { cache: 'no-store' },
  );
  expect(response.status).toBe(200);
  const session = await response.json();
  if (!hasStringProperty(session, 'hostId'))
    throw new Error('Control server returned an invalid dev session.');
  return session.hostId;
}

export async function registryArtifactIds(
  port: number,
): Promise<{ hosts: string[]; apps: string[] }> {
  const response = await fetch(`http://localhost:${port}/registry.json`, {
    cache: 'no-store',
  });
  expect(response.status).toBe(200);
  const registry = await response.json();
  if (!hasRegistryIds(registry))
    throw new Error('Control server returned an invalid registry.');
  return {
...
export function assertSingleComponentDeclaration(
  path: string,
  contents: string,
): void {
  const componentCount =
    angularComponentCount(contents) + reactComponentCount(contents);
  if (componentCount > 1)
    throw new Error(
      `${path} contains ${componentCount} component declarations`,
    );
}
...
function nonInteractivePrompts(): Pick<
  AtlasPrompter,
  'interactive' | 'input' | 'select'
> {
  return {
    interactive: false,
    async input() {
      throw new Error('Unexpected interactive input prompt.');
    },
    async select() {
      throw new Error('Unexpected interactive selection prompt.');
    },
  };
}
...
function hasStringProperty(
  ...
  if (typeof value !== 'object' || value === null || !(property in value))
    return false;
  return (
    typeof Object.getOwnPropertyDescriptor(value, property)?.value === 'string'
  );
}

function hasManifestIds(
  value: unknown,
): value is { apps: Array<{ id: string }> } {
  return (
    typeof value === 'object' &&
    value !== null &&
    'apps' in value &&
...
function hasRegistryIds(
  value: unknown,
): value is { hosts: Array<{ id: string }>; apps: Array<{ id: string }> } {
  return (
    typeof value === 'object' &&
    value !== null &&
    'hosts' in value &&
    Array.isArray(value.hosts) &&
    value.hosts.every((manifest) => hasStringProperty(manifest, 'id')) &&
    'apps' in value &&
    Array.isArray(value.apps) &&
    value.apps.every((manifest) => hasStringProperty(manifest, 'id'))
  );
