...
export const COMMAND_HELP: Readonly<Record<string, CommandHelp>> = {
  ...
  dev: {
    ...
    options: [
      ...
      { label: "--port <number>", description: "Host browser port or app framework port (defaults to next unused port from host 4200 or app 4201)" },
...
function generationProjectHelp(type: "host" | "app", resource: string): CommandHelp {
  return {
    ...
    options: [
      ...
      { label: "--style <format>", description: "Angular stylesheet format: css, scss, sass, or less; prompted when omitted in interactive mode" },
      { label: "--port <number>", description: `Dev-server port; defaults to next unused port from ${type === "host" ? 4200 : 4201}` },
