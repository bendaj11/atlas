...
export async function prepareStaticRegistry(
  ...
  const hostIds =
    manifest.channel === 'production'
      ? affectedHostIds(manifest, registry)
      : [];
  for (const hostId of hostIds) {
    ...
    await writeJson(
      outputDirectory,
      `hosts/${hostId}/deployments/${catalog.revision.replace(':', '-')}.json`,
      catalog,
    );
...
export function createHostCatalog(
  ...
  const host = selectProductionArtifact(
    registry.hosts,
    registry.selections?.hosts?.[hostId],
  );
  if (!host || host.id !== hostId) return undefined;
  const selectedApps = selectProductionApps(
    registry.apps,
    registry.selections?.apps,
  );
  ...
  const revision = `sha256:${createHash('sha256')
    .update(JSON.stringify(sortObjectKeys(catalogContent)))
    .digest('hex')}`;
...
export async function prepareStaticRollback(options: {
  ...
  const candidates = [...options.current.hosts, ...options.current.apps].filter(
    (manifest) =>
      manifest.id === options.artifactId &&
      manifest.channel === 'production' &&
  ...
  if (candidates.length === 0)
    throw new Error(
      `No production build found for Atlas artifact "${options.artifactId}" at version "${options.version}".`,
    );
  if (candidates.length > 1)
    throw new Error(
      `Atlas artifact "${options.artifactId}" has multiple builds for version "${options.version}". Pass --build-id.`,
    );
...
export async function prepareStaticPrRemoval(options: {
  ...
  if (!Number.isSafeInteger(options.prNumber) || options.prNumber < 1) {
    throw new Error(
      'Atlas PR removal requires a positive integer pull-request number.',
    );
  }
  return prepareStaticPrCleanup(
    options,
    (manifest) => manifest.prNumber === options.prNumber,
  );
}
...
async function prepareStaticPrCleanup(
  ...
  const removed = candidates.filter(
    (manifest) =>
      artifactIds.has(manifest.id) &&
      manifest.channel === 'pr' &&
      shouldRemove(manifest),
  );
  const registry: AtlasStaticRegistry = {
    ...options.current,
    updatedAt: options.updatedAt ?? new Date().toISOString(),
    hosts: options.current.hosts.filter(
      (manifest) => !removed.includes(manifest),
    ),
    apps: options.current.apps.filter(
      (manifest) => !removed.includes(manifest),
    ),
  };
  const baseRevision = registryRevision(options.current);
...
function latestArtifactTimestamp(
  manifests: readonly AtlasArtifactManifest[],
): string {
  return manifests
...
function publishArtifact(
  ...
  const registry: AtlasStaticRegistry = {
    ...
    hosts: replaceArtifact(
      current?.hosts ?? [],
      manifest.kind === 'host' ? manifest : undefined,
    ),
    apps: replaceArtifact(
      current?.apps ?? [],
      manifest.kind === 'app' ? manifest : undefined,
    ),
    selections: {
...
function selectArtifact(
  current: AtlasStaticRegistry,
  selected: AtlasArtifactManifest,
  updatedAt: string,
): AtlasStaticRegistry {
  return {
    ...current,
    ...
    selections: {
      hosts: {
        ...
        ...(selected.kind === 'host'
          ? { [selected.id]: selectionOf(selected) }
          : {}),
      },
      apps: {
...
function selectionOf(
  manifest: AtlasArtifactManifestBase,
): AtlasProductionSelection {
  return { version: manifest.version, buildId: manifest.buildId };
}

function replaceArtifact<T extends AtlasArtifactManifest>(
  manifests: T[],
  ...
  return [
    ...manifests.filter(
      (manifest) =>
        artifactKey(manifest) !== key && !samePullRequest(manifest, published),
    ),
    published,
  ].sort(
    (left, right) =>
      left.id.localeCompare(right.id) || compareNewestFirst(left, right),
  );
}
...
function supersededPullRequestArtifacts(
  ...
  const manifests =
    published.kind === 'host' ? (current?.hosts ?? []) : (current?.apps ?? []);
  return manifests.filter(
    (manifest) =>
      samePullRequest(manifest, published) &&
      artifactKey(manifest) !== artifactKey(published),
  );
}

function samePullRequest(
  left: AtlasArtifactManifestBase,
  right: AtlasArtifactManifestBase,
): boolean {
  return (
    left.channel === 'pr' &&
    right.channel === 'pr' &&
...
async function writeArtifactIndex(
  ...
  if (published.kind === 'host') {
    const index: AtlasHostIndex = {
      ...
      id: published.id,
      updatedAt,
      manifests: registry.hosts
        .filter((candidate) => candidate.id === published.id)
...
function affectedHostIds(
  ...
  for (const app of registry.apps) {
    for (const hostId of app.supportedHosts)
...
function selectProductionApps(
  ...
  return appIds.flatMap((id) => {
    const selected = selectProductionArtifact(
...
function selectProductionArtifact<T extends AtlasArtifactManifest>(
  ...
  const production = manifests.filter(
...
function includeHostApps(
  hostId: string,
  selectedApps: AtlasManifest[],
): AtlasManifest[] {
  return selectedApps
    .filter((manifest) =>
...
function assertStaticRegistry(registry: AtlasStaticRegistry | undefined): void {
  ...
  if (
    ...
    !Array.isArray(registry.hosts) ||
    !Array.isArray(registry.apps)
  ) {
    throw new Error(
      'The static Atlas registry is malformed or uses an unsupported schema version.',
    );
  }
  ...
  if (registry.revision && registry.revision !== actualRevision) {
    throw new Error(
      `The static Atlas registry revision is invalid. Expected "${registry.revision}", but its contents produce "${actualRevision}".`,
    );
  }
}
...
function compareNewestFirst(
  ...
  const version = right.version.localeCompare(left.version, undefined, {
    numeric: true,
    sensitivity: 'base',
  });
  return (
...
async function writeJson(
  ...
  if (
    ...
    throw new Error(
      `Registry path "${relativePath}" escapes its output directory.`,
    );
  }
