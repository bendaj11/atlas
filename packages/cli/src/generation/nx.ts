...
import { addUniqueString } from './files.js';
...
export function atlasConfigNxTarget(
  ...
  return {
    ...nxTarget(packageManager, cwd, 'atlas:config'),
...
export async function ensureDelegatedNxTargets(
  ...
  const projectRoot = normalizedProjectRoot(workspaceRoot, root);
  const previousProjectRoot = projectRootFromNxProject(
    project,
    workspaceRoot,
    root,
  );
  if (previousProjectRoot !== projectRoot) {
    throw new Error(
      staleNxProjectRootMessage(project, previousProjectRoot, projectRoot),
    );
  }
...
function ensureAtlasConfigTarget(
  targets: Record<string, unknown>,
  projectName: string,
): void {
...
function preserveNativeDevTarget(
  targets: Record<string, unknown>,
  projectName: string,
): void {
  if (!targets.dev) return;
  if (!targets.serve || delegatesToDevTarget(targets.serve, projectName))
...
function isOutdatedAtlasConfigTarget(value: unknown): boolean {
  ...
  return (
    ...
    !declaresAtlasConfigOutput(target)
  );
}
...
function projectRootFromNxProject(
  project: Record<string, unknown>,
  workspaceRoot: string,
  root: string,
): string {
  const configuredRoot =
    typeof project.root === 'string' && project.root ? project.root : undefined;
