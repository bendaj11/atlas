...
export function validateIdentifier(
  ...
  if (
    ...
    addIssue(
      issues,
      path,
      `Expected ${label} to contain only letters, numbers, dots, dashes, and underscores, without traversal.`,
    );
    return false;
  }
...
export function validateSemanticVersionRange(
  value: unknown,
  path: string,
  issues: AtlasValidationIssue[],
): void {
  if (
    typeof value === 'string' &&
    value.trim() !== '' &&
    !SEMANTIC_VERSION_RANGE.test(value)
  ) {
    addIssue(issues, path, 'Expected a semantic version range such as ^1.2.3.');
  }
}

export function validateSha256Integrity(
  value: unknown,
  path: string,
  issues: AtlasValidationIssue[],
): void {
  if (
    value !== undefined &&
    (typeof value !== 'string' || !SHA_256_INTEGRITY.test(value))
  ) {
  ...
  for (const [key, entry] of Object.entries(metadata)) {
    if (!isMetadataValue(entry))
      addIssue(
        issues,
        `${path}.${key}`,
        'Expected a string, number, or boolean metadata value.',
      );
  }
}
