...
function placements(config: AtlasAppConfig): AtlasPlacement[] {
  return [
    ...(config.routes ?? []).map((route, index, routes) => {
      ...
      return {
        id: routePlacementId(route.hostId, path, routes.slice(0, index)),
        kind: 'route' as const,
        hostId: route.hostId,
        route: {
          path: path,
...
function supportedHosts(config: AtlasAppConfig): string[] {
  const hosts = [
    ...new Set(placements(config).map((placement) => placement.hostId)),
  ];
  return hosts.length ? hosts : ['*'];
}

function routePlacementId(
  hostId: string,
  path: string,
  previousRoutes: readonly { hostId: string; path: string }[],
): string {
  const baseId = identifierFromRoute(hostId, path);
  const duplicateCount = previousRoutes.filter(
    (route) => identifierFromRoute(route.hostId, route.path) === baseId,
  ).length;
  return duplicateCount === 0 ? baseId : `${baseId}-${duplicateCount + 1}`;
}

function slotPlacementId(
  hostId: string,
  slotId: string,
  previousSlots: readonly { hostId: string; slotId: string }[],
): string {
  const baseId = identifierFromSlot(hostId, slotId);
  const duplicateCount = previousSlots.filter(
    (slot) => identifierFromSlot(slot.hostId, slot.slotId) === baseId,
  ).length;
  return duplicateCount === 0 ? baseId : `${baseId}-${duplicateCount + 1}`;
}

function identifierFromRoute(hostId: string, path: string): string {
  const value = `${hostId}-${path}`
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, '-')
    .replace(/-+/g, '-')
    .replace(/^-+|-+$/g, '');
