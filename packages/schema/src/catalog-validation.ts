...
export function validateHostCatalog(value: unknown): AtlasValidationIssue[] {
  ...
  if (catalog?.schemaVersion !== '1')
    addIssue(issues, 'schemaVersion', 'Expected schemaVersion to be "1".');
  issues.push(
    ...validateAtlasHostManifest(catalog?.host).map((issue) => ({
      ...issue,
      path: `host.${issue.path}`,
    })),
  );
  ...
  if (hostId && typeof host?.id === 'string' && host.id !== hostId)
    addIssue(
      issues,
      'host.id',
      'Expected selected host id to match catalog hostId.',
    );
  ...
  catalog.apps.forEach((manifest, index) =>
    issues.push(...validateManifest(manifest, `apps.${index}`)),
  );
  validateUniqueManifestIds(catalog.apps, issues);
  if (catalog.widgetProviders !== undefined) {
    if (!Array.isArray(catalog.widgetProviders))
      addIssue(
        issues,
        'widgetProviders',
        'Expected widgetProviders to be an array.',
      );
    else {
      catalog.widgetProviders.forEach((manifest, index) =>
        issues.push(...validateManifest(manifest, `widgetProviders.${index}`)),
      );
      validateUniqueManifestIds(
...
function validateUniqueManifestIds(
  manifests: unknown[],
  ...
  manifests.forEach((manifestValue, manifestIndex) => {
    ...
    if (manifestIds.has(manifest.id)) {
      addIssue(
        issues,
        `${path}.${manifestIndex}.id`,
        `Duplicate app id "${manifest.id}".`,
      );
    } else {
